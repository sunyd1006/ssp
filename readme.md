...
<img src="readme.assets/8db4a3bdfb238da1a1c4431d2b6e075c_b.png" alt="img" style="zoom:50%;" />

Node[] table：`HashMap`类中有一个非常重要的字段，就是 `Node[] table`，即哈希桶数组，明显它是一个Node的数组。Node是`HashMap`的一个内部类，实现了`Map.Entry`接口，本质是就是一个映射(键值对)。上图中的每个黑色圆点就是一个Node对象。
