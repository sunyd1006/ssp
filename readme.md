...
1. Key不可变。可变的key会使得HashCode的值有变化，存储时利用Key计算获取的桶的位置，和获取时根据（变化后的Key）计算的Hashcode的值不一致，从而获取不到。
2. 结构上：数组+链表+红黑树，其中加红黑树是为了提高拉链太长的参训效率
3. 从线程安全上：HashMap、ArrayList等都不是线程安全的。要使用建议用 Collections.
...
- 线程A在扩容resize()时，在还没有复制不完全时，线程B来get(key)，在新数组还未完成copy时，get已存在的key，但值为null，https://stackoverflow.com/questions/18542037/how-to-prove-that-hashmap-in-java-is-not-thread-safe
...
![img](https:////upload-images.jianshu.io/upload_images/2062943-3ee73bf418994bee.png?imageMogr2/auto-orient/strip|imageView2/2/w/1200/format/webp)
...
为什么811：98%的对象，第一轮就死掉了。所以90%是空的，10%是survivor的。
...
3. 在栈内存中为a变量（A对象的引用）开辟空间
   4. 在堆内存为A对象申请空间
   2. 给成员变量进行默认初始化（此时 i=0），同时有一个方法标记，在方法区中创建一个A的方法区，将A的方法区的地址0x01给方法标记
   3. 给成员变量进行显示初始化（此时 i=1）
7. 将A对象的地址值给变量a
...
- volatile对那些可以保证原子性呢：比如简单的get，set，但getAndSet()这类，是不保证原子性的。
- 对于类变量。
...
==总结：树低磁盘IO次数更少，B+树的叶子节点的内容更加连续，磁盘预读机制就能把相邻的数据页读到内存中来，IO又更少。==



B+树的特点：B+树是非二叉树，叶子存数据，非叶子不存数据，存索引。

B+树的非叶子节点是主键，不存储数据，数的层级就更少，就能存更多的索引。能减少磁盘IO，就会少寻道，少旋转，少浪费了时间。

B+树的叶子节点是跟后序节点相连接的，这对范围查找是非常有用的。
...
- 用户对常见的查询需要的字段，比如a为主键，bcd构成1个联合索引。在查询时如果索引+辅助索引叶子节点的内容，就已经满足`select a,b,c,d，where b=1 and c=1 and d=1`就不必回表查询其他内容了。https://www.cnblogs.com/wzc6688/p/11689018.html
...
[MySQL热点问题的总结](https://github.com/ewenliu/Learning_Note/blob/master/Mysql热点问题总结.md)：主要来自于这里
...
sun：http://www.cyc2018.xyz/%E6%95%B0%E6%8D%AE%E5%BA%93/MySQL.html#b-tree-%E5%8E%9F%E7%90%86
...
相比B树，B+树的父节点也必须存在于子节点中，是其中最大或者最小元素，==B+树的节点只存储索引key值，具体信息的地址存在于叶子节点的地址中。这就使以页为单位的索引中可以存放更多的节点==，（sunB+树的内部节点都放在同一盘块中），能减少更多的I/O支出。因此，B+树成为了数据库比较优秀的数据结构，MySQL中MyIsAM和InnoDB都是采用的B+树结构。不同的是前者是非聚集索引，后者主键是聚集索引，所谓聚集索引是物理地址连续存放的索引，在取区间的时候，查找速度非常快，但同样的，插入的速度也会受到影响而降低。聚集索引的物理位置使用链表来进行存储。
...
进程和线程的区别：https://blog.csdn.net/kuangsonghan/article/details/80674777
...
1.两个进程通过一个管道**只能实现单向通信**，如果想双向通信必须再重新创建一个管道。

2.**只能用于具有亲缘关系**的进程间通信，例如父子，兄弟进程。
...
1.命名管道是一个存在于**硬盘上的文件**，而管道是存在于内存中的特殊文件。
...
#### [#](http://www.cyc2018.xyz/Java/Java IO.html#_1-创建选择器)1. 创建选择器

```java
Selector selector = Selector.open();
```

#### [#](http://www.cyc2018.xyz/Java/Java IO.html#_2-将通道注册到选择器上)2. 将通道注册到选择器上
...
#### [#](http://www.cyc2018.xyz/Java/Java IO.html#_3-监听事件)3. 监听事件
...
#### [#](http://www.cyc2018.xyz/Java/Java IO.html#_4-获取到达的事件)4. 获取到达的事件
...
#### [#](http://www.cyc2018.xyz/Java/Java IO.html#_5-事件循环)5. 事件循环
...
### [#](http://www.cyc2018.xyz/Java/Java IO.html#套接字-nio-实例)套接字 NIO 实例
...
阻塞、非阻塞、IO多路复用、信号、异步IO。分为

- 阻塞：第一阶段recvfrom，2cpy
