...
- B+ 树有更低的树高。红黑树的出度一般为2，所以树高更高。B+树出读更大，树更矮胖
- 磁盘访问原理。更矮胖的树，在一个索引节点代表1个页情况下，更矮胖的树，能更少次数的被加载到内存中，IO次数更少。
...
1. B+树的特点：B+树是非二叉树，叶子存数据，非叶子不存数据，存索引。
2. B+树的非叶子节点是主键，不存储数据，数的层级就更少，就能存更多的索引。能减少磁盘IO，就会少寻道，少旋转，少浪费了时间。
3. B+树的叶子节点是跟后序节点相连接的，这对范围查找是非常有用的。
...
### [在浏览器中输入url地址 ->> 显示主页的过程(面试常客)](https://snailclimb.gitee.io/javaguide/#/docs/network/计算机网络?id=五-在浏览器中输入url地址-gtgt-显示主页的过程面试常客)
